...
uint8_t adcRead = 0;  // flag 1-- adc acquizition completed
uint16_t adcCount = 0;
uint16_t refCount = 0;
float adcf= 0.0;
...
int main(void)
{
  ...
  // testTemp2V();
  state.mode = HEATING;
  state.preSetTemp = 300;// cfgGetEncTick(); TODO
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pins : ENC_SW_Pin Han_On_Pin */
  GPIO_InitStruct.Pin = ENC_SW_Pin|Han_On_Pin;
 ...
 void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
 ...
 {
	 UNUSED(htim);
     heaterOff();// turn off heater before ADC start.
